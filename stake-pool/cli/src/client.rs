use {
    ...
    gemachain_program::{borsh::try_from_slice_unchecked, program_pack::Pack, pubkey::Pubkey},
    gpl_stake_pool::{
        stake_program,
...
pub(crate) fn get_stake_accounts_by_withdraw_authority(
    ...
    rpc_client
        .get_program_accounts_with_config(
            &stake_program::id(),
            #[allow(clippy::needless_update)] // TODO: Remove after updating to gemachain >=1.6.10
            ...
            accounts
                ...
                .filter_map(
                    |(address, account)| match deserialize(account.data.as_slice()) {
                        Ok(stake_state) => Some((address, account.carats, stake_state)),
                        Err(err) => {
