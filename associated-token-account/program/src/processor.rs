...
pub fn process_create_associated_token_account(
    ...
    let gpl_token_mint_info = next_account_info(account_info_iter)?;
    let system_program_info = next_account_info(account_info_iter)?;
    let gpl_token_program_info = next_account_info(account_info_iter)?;
    let gpl_token_program_id = gpl_token_program_info.key;
    ...
    let associated_token_account_signer_seeds: &[&[_]] = &[
        ...
        &gpl_token_program_id.to_bytes(),
        &gpl_token_mint_info.key.to_bytes(),
        &[bump_seed],
    ];
    ...
    if let Some(rent_sysvar_info) = rent_sysvar_info {
        ...
        invoke(
            &gpl_token::instruction::initialize_account3(
                gpl_token_program_id,
                associated_token_account_info.key,
                gpl_token_mint_info.key,
                wallet_account_info.key,
